...
class Clientarea extends CI_Controller {
	...
	function server() {
		...
		$config['base_url'] = base_url().'clientarea/server/index';
	...
	function create() {
		if (isset($_POST['submit'])) {
			...
			$tgl = date('Y-m-d'); 
			$tgl_expire = date('Y-m-d', strtotime($tgl. ' + 30 days'));
			$email = $this->input->post('email');
			...
			$hargas = $this->db->query("SELECT * FROM server WHERE server_id = $server_id ORDER BY harga ASC")->row_array();
			$harga = $hargas['harga'];
			$kapas = $this->db->query("SELECT * FROM server WHERE server_id = $server_id ORDER BY kapasitas ASC")->row_array();
			$kapasitas = $kapas['kapasitas'];
			$saldo = $balance-$harga;
			if ($saldo < 0){
			echo "<script>alert('Saldo Tidak Mencukupi!');</script>";
			header("Refresh: 0; URL=" . base_url('clientarea/server') . "");
			}else{
			 if ($kapasitas == 0){
			     echo "<script>alert('Maaf Kapasitas Server Telah Tercapai!');</script>";
			     header("Refresh: 0; URL=" . base_url('clientarea/server') . "");
			 }else{
			  if ($exec_account->num_rows() > 0 ) {
				...
				header("Refresh: 3; URL=" . base_url('clientarea/server') . "");
			  } else {
				...
				$sisax = ($kapasitas - '1');
				$this->db->query("UPDATE server SET kapasitas ='$sisax' WHERE server_id ='$server_id'");
	...
	function account() {
		...
		$config['last_tag_open'] = "<li>";
		$config['last_tag_close'] = "</li>";
		$config['base_url'] = base_url().'clientarea/account/index';
		$config['total_rows'] = $jml->num_rows();
		$config['per_page'] = '10';
		$config['first_page'] = '&laquo;';
		$config['last_page'] = '&raquo;';
	...
	function deposit() {
		if (isset($_POST['submit'])) {
		  ...
		  $get_member_ids = $this->model_member->get_member($get_email)->row_array();
		  $parsx_id = $get_member_ids['member_id'];
		  $this->db->where('member_id' , $parsx_id);
          $checks = $this->db->get('invoice');
          if ($checks->num_rows() > 0) {
                echo "<script>alert('Mohon Selesaikan Deposit Sebelumnya Terlebih Dahulu!');</script>";
                header("Refresh: 0; URL=" . base_url('clientarea') . "");
          }else{
